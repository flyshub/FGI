...
class ScoreRepository:
    ...
    def get_scores(self, start_date: str, end_date: str) -> pd.DataFrame:
        return pd.read_sql_query(
            ...
            self._conn, params=[start_date, end_date],
        )

    def get_score_on_date(self, date: str) -> dict | None:
        cur = self._conn.execute("SELECT * FROM scores_daily WHERE date = ?", (date,))
        row = cur.fetchone()
        if not row:
            return None
    ...
    def get_latest_score_date(self) -> str | None:
        row = self._conn.execute("SELECT MAX(date) FROM scores_daily").fetchone()
...
class RawDataRepository:
    ...
    def get_raw_data(self, indicator: str, start_date: str, end_date: str) -> pd.DataFrame:
        return pd.read_sql_query(
            ...
            self._conn, params=[indicator, start_date, end_date],
        )
    ...
    def count_rows(self) -> int:
        return self._conn.execute("SELECT COUNT(*) FROM raw_data").fetchone()[0]
