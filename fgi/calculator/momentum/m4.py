...
class M4Calculator:
    ...
    def calculate_turnover_deviation(self, df: pd.DataFrame) -> pd.DataFrame:
        df["date"] = pd.to_datetime(df["date"])
        df = df.sort_values("date")
        df["ma20_turnover"] = df["turnover_rate"].rolling(window=20, min_periods=1).mean()
        df["deviation"] = (df["turnover_rate"] - df["ma20_turnover"]) / df["ma20_turnover"]
        return df
    ...
    def run(self, date: str, lookback_days: int = None) -> dict:
        ...
        if result.status != DataSourceStatus.HEALTHY:
            self._db.upsert_status(date, "m4", "missing", result.source or "", result.error or "")
            return {"m4": None, "status": "missing"}

        df = result.data
        df = self.calculate_turnover_deviation(df)
        df = self.calculate_percentile(df)

        today = df[df["date"] == date]
        if today.empty:
            self._db.upsert_status(date, "m4", "missing", result.source or "", "No data for date")
            return {"m4": None, "status": "missing"}

        percentile = today["percentile"].iloc[0]
        if pd.isna(percentile):
            self._db.upsert_status(date, "m4", "missing", result.source or "", "Insufficient data")
            return {"m4": None, "status": "missing"}

        score = self.calculate_score(percentile)

        self._db.upsert_raw_data(date, "m4_deviation", today["deviation"].iloc[0])
        self._db.upsert_raw_data(date, "m4_percentile", percentile)
        self._db.upsert_score(date, {"M4": score})
        self._db.upsert_status(date, "m4", "normal", result.source or "")
