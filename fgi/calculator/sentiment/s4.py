...
class S4Calculator:
    ...
    def _try_fetch_from_source(self, start_date: str, end_date: str) -> pd.DataFrame | None:
        ...
        if result.status == DataSourceStatus.HEALTHY and result.data is not None and not result.data.empty:
            for _, row in result.data.iterrows():
                self._db.upsert_raw_data(str(row["date"]), "s4_seal_fund", float(row["seal_fund_sum"]) / 1e8)
            self._db.commit()
            df = result.data
            df["zt_ratio"] = df["seal_fund_sum"]
    ...
    def run(self, date: str, lookback_days: Optional[int] = None) -> dict:
        ...
        if today.empty:
            self._db.upsert_status(date, "s4", "missing", "database", "No data for date")
            return {"s4": None, "status": "missing"}

        percentile = today["percentile"].iloc[0]
        if pd.isna(percentile):
            self._db.upsert_status(date, "s4", "missing", "database", "Insufficient data")
            return {"s4": None, "status": "missing"}
        ...
        self._db.upsert_status(date, "s4", "normal", "database")
